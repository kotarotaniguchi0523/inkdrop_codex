...
Pull Requestでは変更ファイルを最初に分類し、関係する品質ゲート、依存関係レビュー、CodeQL、
ネイティブCredential Helper Buildだけを実行します。Release Archiveは生成しません。`main`から
**Release** Workflowを手動実行する前に、HEAD CommitがRepository Ownerの作成で、GitHub上の署名表示が
有効な**Verified**であることを確認します。それ以外のCommitはWorkflowが拒否します。Workflowは
`inkdrop-codex-release-bundle`を一度だけBuildし、保護された`inkdrop-production` Environmentを通して、
二度目のEnvironment承認や再Buildを行わずに同一ArchiveへProvenanceを付与して公開します。Version Tagは
公開時にだけ作成します。Inkdrop Registryへの公開は、GitHub Release検証後に行う別の手動工程です。
